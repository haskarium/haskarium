...
makeCreatures :: Window -> StdGen -> [SpeciesType] -> (StdGen, [Creature])
makeCreatures window g = foldr makeCreatures' (g, [])
  where
    ((minX, minY), (maxX, maxY)) = window
    makeCreatures' speciesType (g0, creatures) = (g5, c : creatures)
      where
        ...
        (tr, g4) = case speciesType of
            SCentipede ->
                randomR (-pi / 34, -pi / 30) g3
            _ ->
                randomR (pi / 4, pi / 2) g3
        (s', g5) = case speciesType of
            SCentipede ->
